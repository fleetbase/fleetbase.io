...
Fleetbase marketing site, documentation, developer portal, API reference generation, blog, and public content experience.

## Allowed Work

- Documentation, marketing copy, API reference generation, blog, content, and small frontend fixes that follow the Next.js/Fumadocs structure.
- Updates that reflect source-of-truth product behavior from Fleetbase repositories.
- API docs regeneration when Postman/OpenAPI sources change.

## Restricted Work

- Publishing, deployment, analytics, production environment, or credential changes without explicit approval.
- Inventing product, pricing, API, or integration behavior without source-of-truth validation.
- Manual API reference edits that should instead come from `fleetbase/postman` generated sources.
...
- `pnpm install`
- `git submodule update --init --recursive` if the Postman submodule is missing

## Test Commands

- Lint: `pnpm lint`
- Build: `pnpm build`
- Generate API docs: `pnpm generate:api-docs`
- Dev server: `pnpm dev`
