...
![Algorithm form — formula field with the list of available variables and worked examples](/images/screenshots/fleet-ops/fleet-ops-service-rate-algorithm.webp)

## How an Algorithm Rate Is Evaluated

When an order is quoted against an algorithm rate:

1. The order is summarised into a set of **variables** (distance, time, stop count, parcel count, etc. — see below)
2. Each `{variable}` placeholder in your formula is replaced with its numeric value
3. Any supported function calls (`max`, `min`, `ceil`, `floor`, `round`) are resolved
4. The remaining math is evaluated
5. The result is rounded to 2 decimal places (typical for currency) and added to the quote's base fee

Results are always numeric. If the expression fails to evaluate (bad syntax, unknown variable, division by zero), the rate falls back to whatever else is on the rate (base fee, parcel fees).
...
Before saving, Fleet-Ops can validate that an expression is **computable** — it runs the expression against a set of representative values to confirm the result is numeric. The validator evaluates your formula against:
