...
The **Fulfillment** module in Pallet handles the movement of goods into and out of your [Warehouses](/docs/pallet/warehouses/overview).
...
1. You create a Purchase Order to request goods from a [Supplier](/docs/pallet/suppliers/overview).
...
4. Pallet automatically updates the [Inventory](/docs/pallet/inventory/overview) levels, adding the new stock to the designated receiving [Bin](/docs/pallet/warehouses/zones-aisles-racks-bins).
...
### 1. Order Allocation
When a Sales Order is created, Pallet checks the current [Inventory](/docs/pallet/inventory/overview) levels. If sufficient stock is available, Pallet "allocates" the stock to the order. This prevents the same item from being sold twice.
...
At this point, the order is ready for dispatch. If you are using [Fleet-Ops](/docs/fleetops), Pallet can automatically hand the order over to the dispatch engine to assign a driver for final-mile delivery.
