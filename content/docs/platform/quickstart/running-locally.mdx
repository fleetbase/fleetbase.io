...
| **Redis** | `cache:6379` (internal only) | Cache and job queue |
| **SocketCluster** | ws://localhost:38000 | Real-time WebSocket server |

Redis is not published to the host. Other containers reach it at `cache:6379` over the Compose network. To inspect it, run `redis-cli` inside the container with `docker compose exec cache redis-cli`.
...
<Tabs items={['Backend', 'Frontend']}>
  <Tab value="Backend">
    ...
    **OAuth sign-in** — only needed if you offer Google, Microsoft, GitHub, or Apple sign-in. Providers are normally configured in **Admin → Auth Config → OAuth Sign-in**; set this when the API's public URL differs from `APP_URL`:

    ```yaml
          OAUTH_REDIRECT_BASE: ""     # public origin of the API, e.g. https://api.example.com
    ```

    See [OAuth Sign-in](/docs/platform/system-setup/oauth-sign-in) for the full list of variables.
