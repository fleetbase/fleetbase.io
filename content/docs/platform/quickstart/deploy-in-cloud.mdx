...
<Callout type="info">
  For larger self-hosted or on-premise deployments, including Kubernetes HA, storage growth, disaster recovery, and enterprise production sizing, see [Infrastructure Sizing](/docs/platform/quickstart/infrastructure-sizing).
</Callout>
...
- [ ] OAuth callback URLs registered with each provider, if you offer provider sign-in
...
**Provider sign-in fails with a redirect URI mismatch**
The callback URL Fleetbase builds must match the one registered with the provider exactly. Behind a proxy or load balancer whose public URL differs from `APP_URL`, set `OAUTH_REDIRECT_BASE` to the API's public origin, then copy the Callback URL shown in **Admin → Auth Config → OAuth Sign-in**. See [OAuth Sign-in](/docs/platform/system-setup/oauth-sign-in).
